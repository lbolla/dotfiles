...
alias find-dangling-links="find . -xtype l"
...
# alias w=workon
alias youtube-dl-song="youtube-dl -i --force-ipv4 -x -f 'bestaudio[ext=m4a]/best[ext=mp4]/best' -o '%(title)s.%(ext)s'"
alias youtube-dl-playlist="youtube-dl -i --force-ipv4 -x -f 'bestaudio[ext=m4a]/best[ext=mp4]/best' -o '%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s'"
...
_image_fts=(JPG JPEG)
for ft in $_image_fts; do alias -s $ft=xdg-open; done
